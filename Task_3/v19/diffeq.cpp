...
#define mu_a 0
#define lambda_a 1 // u(0.5)=-0.5
#define psi_a 0

#define mu_b 1
#define lambda_b 1 // u'(1)+u(1)=-1
#define psi_b -1
...
double q(double x) { return 2; }

double g(double x) { return pow(cos(M_PI * x), 2); }

double *_minus(double *a, double *b, int n)
{
...
double *solv_sys(double *a, double *b, double *c, double *d, int n)
{
...
int main()
{
    ...
    int num_vals = sizeof(N) / sizeof(*N);
    ...
    for (int i = 0; i < num_vals - 1; i++)
    {
        ...
        double calc1 = norma(_minus(u_, v_, N[i]), N[i]);
        ...
        u_ = u(a_0, b_0, N[i + 1]);
        v_ = v(a_0, b_0, N[i + 1]);
        double calc2 = norma(_minus(u_, v_, N[i + 1]), N[i + 1]);
    ...
    for (int i = 0; i < num_vals - 1; i++)
    {
        double *u_ = u(a_0, b_0, N[i]);
        double *v_ = v(a_0, b_0, N[i]);
        double calc1 = norma1(_minus(u_, v_, N[i]), N[i]);
        ...
        u_ = u(a_0, b_0, N[i + 1]);
        v_ = v(a_0, b_0, N[i + 1]);
        double calc2 = norma1(_minus(u_, v_, N[i + 1]), N[i + 1]);
    ...
    for (int i = 0; i < num_vals - 1; i++)
    {
        double *u_ = u(a_0, b_0, N[i]);
        double *v_ = v(a_0, b_0, N[i]);
        double calc1 = norma2(_minus(u_, v_, N[i]), N[i]);
        ...
        u_ = u(a_0, b_0, N[i + 1]);
        v_ = v(a_0, b_0, N[i + 1]);
        double calc2 = norma2(_minus(u_, v_, N[i + 1]), N[i + 1]);
